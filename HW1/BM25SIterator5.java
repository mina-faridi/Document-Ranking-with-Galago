...
public class BM25SIterator5 extends ScoringFunctionIterator implements DeltaScoringIterator {
  ...
  public BM25SIterator5(NodeParameters np, LengthsIterator ls, CountIterator it)
          throws IOException {
    super(np, ls, it);
  ...
  private double score(double count, double length) {
    double numerator = (k + 1) * (count / (1 - b + (b * length / avgDocLength)) + 0.5);
    double denominator = k + (count / (1 - b + (b * length / avgDocLength)) + 0.5);
